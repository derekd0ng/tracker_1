...
function cacheWrite(all: Record<string, DiaryEntry>) {
...
function computeStreak(entries: Record<string, DiaryEntry>): number {
  ...
  while (entries[localDateStr(d)]) { streak++; d.setDate(d.getDate() - 1); }
  return streak;
}

export default function DiaryTab() {
  const [date, setDate] = useState(localDateStr());
  const [all, setAll] = useState<Record<string, DiaryEntry>>({});
  const [loading, setLoading] = useState(true);
  const [saved, setSaved] = useState(false);
  const [expandedPrompts, setExpandedPrompts] = useState(false);
  const saveTimeout = useRef<ReturnType<typeof setTimeout> | null>(null);
  const apiSaveTimeout = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Load from API; migrate any existing localStorage entries
  useEffect(() => {
    (async () => {
      try {
        const entries: DiaryEntry[] = await api.get('/api/diary');
        const byDate: Record<string, DiaryEntry> = {};
        entries.forEach(e => { byDate[e.date] = e; });

        // One-time migration from localStorage
        const local = (() => {
          try { return JSON.parse(localStorage.getItem(STORAGE_KEY) ?? '{}') as Record<string, any>; }
          catch { return {}; }
        })();
        const toMigrate = Object.entries(local).filter(([d]) => !byDate[d]);
        if (toMigrate.length > 0) {
          await Promise.all(toMigrate.map(([d, e]: [string, any]) =>
            api.put(`/api/diary/${d}`, { freeText: e.freeText ?? '', prompts: e.prompts ?? {} })
              .then((saved: DiaryEntry) => { byDate[saved.date] = saved; })
              .catch(() => {})
          ));
        }
    ...
    return () => {
      ...
      if (apiSaveTimeout.current) clearTimeout(apiSaveTimeout.current);
    };
  }, []);
  ...
  function persist(updated: DiaryEntry) {
    ...
    // Debounce API writes to avoid flooding on every keystroke
    if (apiSaveTimeout.current) clearTimeout(apiSaveTimeout.current);
    apiSaveTimeout.current = setTimeout(() => {
      api.put(`/api/diary/${updated.date}`, {
        freeText: updated.freeText,
        prompts: updated.prompts ?? {},
      }).catch(err => console.error('diary save:', err));
    }, 800);
  }
  ...
  if (loading) return (
    <div style={{ padding: 40, color: 'var(--text-muted)', fontFamily: "'JetBrains Mono', monospace", fontSize: 13 }}>
      Loading…
    </div>
  );
    ...
    <div className="diary-theme" style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 800 }}>
      ...
      <div className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px' }}>
        ...
        <button
          ...
          style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: '1.1rem', cursor: date >= localDateStr() ? 'default' : 'pointer', padding: '4px 8px', fontFamily: 'inherit', borderRadius: 4, opacity: date >= localDateStr() ? 0.3 : 1, transition: 'color 0.12s' }}
