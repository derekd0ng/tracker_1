...
import {
  ...
  exportData, exportAsXlsx, importData,
} from '../../storage';
...
// ── HomeTab ───────────────────────────────────────────────────────────────────
export default function HomeTab({ onNavigate, user, onLogout }: Props) {
    ...
    <div style={{ display:'flex', flexDirection:'column', gap: 16, position:'relative' }}>
      ...
      <div style={{ borderBottom:'1px solid #1e1e1e', paddingBottom: 14, display:'flex', alignItems:'center', position:'relative', zIndex: 10 }}>
        ...
        <div style={{ display:'flex', alignItems:'baseline', gap: 10 }}>
          ...
          <span style={{ fontSize: 11, color:'#555', fontFamily:"'JetBrains Mono',monospace", whiteSpace:'nowrap' }}>{dateLabel}</span>
        </div>
...
{/* Cards */}
      ...
      <div ref={settingsRef} style={{ position: 'fixed', bottom: 24, left: 24, zIndex: 300 }}>
        {settingsOpen && (
          <div style={{
            position: 'absolute', bottom: 'calc(100% + 8px)', left: 0,
            background: '#0f0f0f', border: '1px solid #2a2a2a',
            borderRadius: 6, zIndex: 300, minWidth: 210,
            boxShadow: '0 8px 24px rgba(0,0,0,0.5)',
